...
async fn swap(mut sdk: sdk_on_chain::DarklakeSDK, user_keypair: Keypair) -> Result<()> {
    ...
    // Swap tx
    let (mut swap_tx_, order_key, min_out, salt) = sdk.swap_tx(
            token_mint_x,
            token_mint_y,
            1_000,
            1,
    ...
    let (mut finalize_tx) = sdk.finalize_tx(order_key, unwrap_wsol, min_out, salt, None).await?;
...
async fn add_liquidity(mut sdk: sdk_on_chain::DarklakeSDK, user_keypair: Keypair) -> Result<()> {
    ...
    let (mut add_liquidity_tx) = sdk
        .add_liquidity_tx(token_mint_x, token_mint_y, 1_000, 1_000, 20, user_keypair.pubkey())
        .await?;
...
async fn manual_swap_to_sol(
    ...
    println!("Darklake DEX SDK - Manual Swap To SOL Example");
    println!("==============================================");
...
async fn swap_from_sol(mut sdk: sdk_on_chain::DarklakeSDK, user_keypair: Keypair) -> Result<()> {
    ...
    let (mut swap_tx_, order_key, min_out, salt) = sdk.swap_tx(token_mint_x, token_mint_y, 1_000, 1, user_keypair.pubkey()).await?;
    ...
    let (mut finalize_tx) = sdk.finalize_tx(order_key, true, min_out, salt, None).await?;
...
async fn swap_to_sol(mut sdk: sdk_on_chain::DarklakeSDK, user_keypair: Keypair) -> Result<()> {
    println!("Darklake DEX SDK - Swap To SOL Example");
    println!("======================================");
...
async fn manual_add_liquidity_sol(
    ...
    println!("Darklake DEX SDK - Manual Add Liquidity with SOL");
    println!("================================================");
...
async fn manual_remove_liquidity_sol(
    ...
    println!("Darklake DEX SDK - Manual Remove Liquidity with SOL");
    println!("==================================================");
    ...
    let unwrap_instructions =
        utils::get_unwrap_wsol_to_sol_instructions(user_keypair.pubkey())?;
...
async fn main() -> Result<()> {
    ...
    match args[1].as_str() {
        ...
        "swap" => {
            ...
            swap(sdk, load_wallet_key()?).await
        }
        ...
        "add_liquidity" => {
            ...
            add_liquidity(sdk, load_wallet_key()?).await
        }
        ...
        "remove_liquidity" => {
            ...
            remove_liquidity(sdk, load_wallet_key()?).await
        }
        ...
        "swap_from_sol" => {
            ...
            swap_from_sol(sdk, load_wallet_key()?).await
        }
        "swap_to_sol" => {
            println!("Running swap_to_sol()...");
            swap_to_sol(sdk, load_wallet_key()?).await
        }
